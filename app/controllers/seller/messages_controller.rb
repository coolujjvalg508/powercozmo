...
class Seller::MessagesController < Seller::BaseController
  ...
  def create
    ...
    if (@receipt.errors.blank?)
      ...
      redirect_to seller_message_path(conversation)
    else    	
      render :action => :new
    end
  end
  ...
  def remove
    if params[:seller_message_ids].present? && params[:seller_message_ids].is_a?(Array) && params[:seller_message_ids].map(&:to_i).length > 0  		
  		conversations = mailbox.conversations.where(id: params[:seller_message_ids].map(&:to_i).compact.uniq)
  		conversations.each do |conversation|
  			conversation.move_to_trash(current_user)
  		end
  		flash[:notice] = "Successfully destroyed #{params[:seller_message_ids].map(&:to_i).count} messages"
  	end
  	redirect_to :back
		rescue ActionController::RedirectBackError
  		redirect_to seller_dashboard_path  	
  ...
  def trash
    conversation.move_to_trash(current_user)
    redirect_to inbox_seller_messages_path
  end
