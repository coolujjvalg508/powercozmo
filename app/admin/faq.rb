ActiveAdmin.register Faq do
  menu parent: 'CMS', :label => 'FAQs', priority: 2, if: proc{ (current_admin_user.has_permission('cms_read') || current_admin_user.has_permission('cms_write') || current_admin_user.has_permission('cms_delete'))}
  ...
  controller do 
    ...
    def sort      
      ...
      @faq.set_list_position(params[:position].to_i)
      render :json => {message: 'Position updated'}
    end
  end

  action_item :back, only: :show do    
    link_to "Back", admin_faqs_path, method: :get
  end

  batch_action "Update 'Active' for", form: { active: %w[Yes No] } do |ids, inputs|
    # inputs is a hash of all the form fields you requested   
    Faq.where(id: ids).update_all(active: inputs[:active])
